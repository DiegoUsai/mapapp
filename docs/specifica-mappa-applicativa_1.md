...
Sopra le viste sono disponibili filtri e una ricerca testuale. I filtri per dominio, fornitore e contratto devono elencare **solo i valori effettivamente in uso** — per esempio, tra i domini vanno mostrati unicamente quelli associati ad almeno un applicativo, non l'intero elenco dei domini censiti — così da non affollare l'interfaccia con opzioni che non filtrerebbero nulla. La **ricerca testuale** non deve limitarsi ai nomi degli applicativi: deve cercare anche tra i **requisiti** (e, dove sensato, tra i moduli e le integrazioni), così che digitando il nome di un requisito si trovino gli applicativi che lo contengono, non solo quelli il cui nome corrisponde al testo.
...
Tutte le entità devono essere creabili, **modificabili in qualsiasi momento** ed eliminabili dall'interfaccia, restando nel flusso di navigazione, senza pagine di amministrazione separate. La modificabilità vale per **tutte** le entità senza eccezioni, applicativo compreso: dev'essere possibile aggiornare da front-end nome, dominio, contratti collegati e moduli di un applicativo già creato, non solo crearlo. In particolare:
