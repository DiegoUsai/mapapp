...
A questo si affianca un livello di **allineamento semantico** dei domini alle nomenclature ufficiali italiane ed europee (codici COFOG per le funzioni di governo, Eurovoc per i temi dei dati, in coerenza con le linee guida AgID). Questo allineamento non contraddice la scelta dei confini labili: i codici standard sono etichette descrittive facoltative che arricchiscono il dominio e ne rafforzano la riconoscibilità istituzionale, non vincoli che impongono a un requisito di stare in una casella e una sola. La distinzione tra domini **verticali** e **trasversali core** ha lo stesso spirito: descrive la natura d'uso del dominio (specifico di un ambito o condiviso tra più aree) senza trasformarsi in una gerarchia rigida.
...
**Dominio** — rappresenta un'area applicativa. Campi: nome, colore (usato per distinguere i nodi nella mappa), tipo di dominio (**verticale** o **trasversale core**: i domini trasversali, come la gestione documentale, l'identità digitale o i pagamenti, sono usati da più aree; i verticali sono specifici di un ambito), e un riferimento semantico agli standard di classificazione della PA italiana ed europea. Quest'ultimo non è un testo libero: va scelto da un **elenco precompilato** costruito sulle ontologie ufficiali — i codici **COFOG** (Classification of the Functions of Government) e/o gli **URI Eurovoc** — così che ogni dominio sia agganciato a una voce standard e non a una descrizione arbitraria. L'indicazione verticale/trasversale resta una caratterizzazione libera e utile, ma la classificazione semantica vera e propria passa dalla selezione di una voce dell'elenco ontologico. Questo allinea i domini alle nomenclature ufficiali e alle linee guida AgID, rafforzando la credibilità dello strumento verso il cliente. Il dominio non è gerarchico: un applicativo appartiene a un solo dominio, ma requisiti di domini diversi possono essere collegati tra loro.
...
- Nella creazione di un applicativo si scelgono il dominio e uno o più contratti; deve essere possibile creare un nuovo dominio o un nuovo contratto al volo, senza interrompere l'operazione.
