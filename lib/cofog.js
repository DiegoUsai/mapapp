...
// Etichetta "pulita", senza prefisso codice né suffisso "(generico)": adatta a precompilare
// un'etichetta breve (es. nome dominio) a partire dalla voce COFOG scelta.
export function cofogCleanLabel(code) {
  for (const division of COFOG) {
    if (division.code === code) return division.label;
    const group = division.groups.find((g) => g.code === code);
